...
users_list = microservice.list("User", filter_dict={
...
for i in range(50):
    ...
    description = gerador_de_lero_lero(n=4)
    notes = gerador_de_lero_lero(n=20)
    microservice.save({
...
for i in range(200):
    print(i)
    user = random.choice(users_list)
    payload64 = random.choice(photos64)
    microservice = PumpWoodMicroService(
        server_url="http://0.0.0.0:8000/",
        username=user["username"], password=user["username"])
    microservice.login()

    temp_date = random_date()

    hash_object = hashlib.sha1(
        "{} | {}".format(user["username"], temp_date).encode())
    pbHash = hash_object.hexdigest()

    app_label = gerador_de_lero_lero(n=4)
    microservice.save({
        "hash": pbHash,
        "model_class": "DescriptionImage",
        "app_label": app_label,
        "image_created_at": temp_date,
        "image64": payload64})



microservice.list("DescriptionImage")
